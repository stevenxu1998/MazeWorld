...
from RL_brain_Qlearning import rlalgorithm as rlalg1
from RL_brain_SARSA import rlalgorithm as rlalg2
from RL_brain_VI import rlalgorithm as rlalg3
from RL_brain_PI import rlalgorithm as rlalg4
import numpy as np
import pandas as pd
import sys
...
def update2(env, RL, data, episodes=50):
    global_reward = np.zeros(episodes)
    data['global_reward']=global_reward
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            # RL learn from this transition
            # and determine next state and action
            state =  RL.learn(str(state), action, reward, str(state_))
...
def update3(env, RL, data, episodes=50):
    global_reward = np.zeros(episodes)
    data['global_reward']=global_reward
    delta = 0
    for episode in range(episodes):
        ...
        while True:
            ...
            state,delta =  RL.learn(str(state), action, reward, str(state_), delta)
...
def update4(env, RL, data, episodes=50):
    global_reward = np.zeros(episodes)
    data['global_reward']=global_reward
    delta = 0
    for episode in range(episodes):
...
if __name__ == "__main__":
    sim_speed = 0.05 #orginal speed is 0.05
    ...
    data1={}
    env1.after(10, update2(env1, RL1, data1, episodes))
    env1.mainloop()
    ...
    # data3={}
    # env3.after(10, update3(env3, RL3, data3, episodes))
    # env3.mainloop()
    # experiments = [(env3,RL3, data3)]

    # env4 = Maze(agentXY,goalXY,wall_shape,pits)
    # RL4 = rlalg4(actions=list(range(env4.n_actions)))
    # data4={}
    # env4.after(10, update4(env4, RL4, data4, episodes))
    # env4.mainloop()
